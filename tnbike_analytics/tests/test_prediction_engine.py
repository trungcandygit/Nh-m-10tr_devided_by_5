...
def test_q1_weekly_q2_only(q1_out):
    ...
    assert len(w) > 0
    # Mỗi tuần × nhóm → ít nhất 5 group × ~13 tuần
...
def test_q1_sku_top20_flag(q1_out):
    ...
    # Mỗi tháng Q2 (3 tháng) × top-20 SKU = 60 rows
...
def test_q2_color_q2_forecast_period(q2_out):
    """color_q2 phải cover 3 tháng Q2-2026."""
...
def test_q3_roc_auc_acceptable(q3_out):
    """ROC-AUC phải > 0.65 sau khi bỏ data leakage (active_in_t3)."""
...
def test_q3_no_leakage_auc_ceiling(q3_out):
    """ROC-AUC phải < 0.99 — nếu ≥0.99 có thể còn data leakage."""
    auc = q3_out["dealer_churn"]["roc_auc_test"].iloc[0]
...
def pytest_sessionfinish(session, exitstatus):
    """Ghi model metrics vào output/test_report.md sau toàn bộ session."""
    try:
        _write_report()
    except Exception:
        pass  # không block nếu lỗi
